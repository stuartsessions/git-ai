...
impl TmpRepo {
    ...
    pub fn new_with_base_commit() -> Result<(Self, TmpFile, TmpFile), GitAiError> {
        ...
        let lines_file = repo.write_file("lines.md", super::LINES, true)?;
        let alphabet_file = repo.write_file("alphabet.md", super::ALPHABET, true)?;
    ...
    ) -> Result<std::collections::BTreeMap<u32, String>, GitAiError> {
    ...
    pub fn reset(
        ...
        match mode {
            super::ResetMode::Hard => args.push("--hard".to_string()),
            super::ResetMode::Soft => args.push("--soft".to_string()),
            super::ResetMode::Mixed => args.push("--mixed".to_string()),
            super::ResetMode::Merge => args.push("--merge".to_string()),
            super::ResetMode::Keep => args.push("--keep".to_string()),
        }
...
mod tests {
    ...
    fn test_repo_specific_env_vars() {
        ...
        assert_eq!(
            repo1.get_env("MY_TEST_VAR"),
            Some("value_for_repo1".to_string())
        );
        assert_eq!(
            repo2.get_env("MY_TEST_VAR"),
            Some("value_for_repo2".to_string())
        );
    ...
    #[test]
    fn test_env_vars_isolated_per_repo() {
        ...
        // Verify the variables are stored and retrievable
        assert_eq!(
            repo.get_env("GIT_AUTHOR_NAME"),
            Some("Test Author".to_string())
        );
        assert_eq!(
            repo.get_env("GIT_AUTHOR_EMAIL"),
            Some("test@example.com".to_string())
        );

        // The environment variables should not leak to the process
        // (This test verifies the isolation without making actual git calls)
    }
}
